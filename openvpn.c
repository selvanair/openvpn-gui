    ...
    case WM_COMMAND:
        ...
        case IDOK:
            if (GetDlgItemTextW(hwndDlg, ID_EDT_AUTH_USER, username, _countof(username)))
            {
                ...
                {
                    show_error_tip(GetDlgItem(hwndDlg, ID_EDT_AUTH_USER), L"Invalid character in username");
                    return FALSE;
                }
            ...
            if (GetDlgItemTextW(hwndDlg, ID_EDT_AUTH_PASS, password, _countof(password)))
            {
                ...
                {
                    show_error_tip(GetDlgItem(hwndDlg, ID_EDT_AUTH_PASS), L"Invalid character in password");
                    SecureZeroMemory(password, sizeof(password));
                    return FALSE;
                }
...
GenericPassDialogFunc(HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
        ...
        case IDOK:
            ...
            {
                show_error_tip(GetDlgItem(hwndDlg, ID_EDT_RESPONSE), L"Invalid character in password");
                SecureZeroMemory(password, sizeof(password));
                return FALSE;
            }
            if (param->flags & FLAG_CR_TYPE_CRV1)
...
PrivKeyPassDialogFunc(HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
        ...
        case IDOK:
            ...
            {
                if (!validate_input(passphrase))
                {
                    show_error_tip(GetDlgItem(hwndDlg, ID_EDT_PASSPHRASE), L"Invalid character in password");
                    SecureZeroMemory(passphrase, sizeof(passphrase));
                    return FALSE;
                }
