...
CreatePopupMenus()
{
    ...
    if (o.num_configs == 1) {
        ...
        if (o.service_only == 0) {
            ...
            AppendMenu(hMenu, MF_STRING, IDM_RESTARTMENU, LoadLocalizedString(IDS_MENU_RESTART));
        ...
        for (i=0; i < o.num_configs; i++) {
            if (o.service_only == 0) {
                ...
                AppendMenu(hMenuConn[i], MF_STRING, IDM_RESTARTMENU + i, LoadLocalizedString(IDS_MENU_RESTART));
...
SetMenuStatus(connection_t *c, conn_state_t state)
{
    if (o.num_configs == 1)
    {
        if (state == disconnected)
        {
            ...
            EnableMenuItem(hMenu, IDM_RESTARTMENU, MF_GRAYED);
        ...
        else if (state == connecting || state == resuming || state == connected)
        {
            ...
            EnableMenuItem(hMenu, IDM_RESTARTMENU, MF_ENABLED);
    ...
    else
    {
        ...
        if (state == disconnected)
        {
            ...
            EnableMenuItem(hMenuConn[i], IDM_RESTARTMENU + i, MF_GRAYED);
        ...
        else if (state == connecting || state == resuming || state == connected)
        {
            ...
            EnableMenuItem(hMenuConn[i], IDM_RESTARTMENU + i, MF_ENABLED);
